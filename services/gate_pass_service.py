...
class GatePassService:
    ...
    def generate_gate_pass(self, current_user: dict, data: dict) -> dict:
        expiration_minutes = int(data["expires_at"])
        if expiration_minutes <= 0:
            raise ValueError("Invalid expiration value.")
        # self.repository.delete_tokens(current_user["email"])
        ...
        return {
            "visitor_name": token_data["visitor_name"],
            "otp": token_id,
            "expires in": time_to_expire
        }

    def validate_gate_pass(self, token_id: str, current_user: dict) -> dict:
        token = self.repository.find_token_by_id(token_id)
        if not token or token["resident"] != current_user["email"]:
